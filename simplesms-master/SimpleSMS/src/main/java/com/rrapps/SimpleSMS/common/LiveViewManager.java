 ...
 * LiveViewManager.registerView(SimpleSMSPreference.THEME, key -> {
 * // Change button color
 * }
 * <p>
...
public abstract class LiveViewManager {
    ...
    private static final HashSet<String> sExcludedPrefs = new HashSet<>(Arrays.asList(
            SimpleSMSPreference.THEME.getKey(),
            SimpleSMSPreference.BACKGROUND.getKey()
    ));
    ...
    /**
     * Register a view to be updated when a SimpleSMSPreference is changed
     ...
     */
    public static void registerView(SimpleSMSPreference preference, Object parent, LiveView view) {
        synchronized (sViews) {
     ...
     */
    public static void refreshViews(SimpleSMSPreference preference) {
