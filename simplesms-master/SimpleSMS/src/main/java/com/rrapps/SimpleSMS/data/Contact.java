package com.rrapps.SimpleSMS.data;
...
//    };
    ...
    public static String formatNameAndNumber(String name, String number, String numberE164) {
        ...
        if (!SmsHelper.isEmailAddress(number)) {
            formattedNumber = PhoneNumberUtils.formatNumber(number, numberE164, SimpleSMSApp.getApplication().getCurrentCountryIso());
        }
    ...
    public synchronized void setNumber(String number) {
        if (!SmsHelper.isEmailAddress(number)) {
            mNumber = PhoneNumberUtils.formatNumber(number, mNumberE164, SimpleSMSApp.getApplication().getCurrentCountryIso());
        } else {
            mNumber = number;
        }
        ...
        private Contact getContactInfoForPhoneNumber(String number) {
            ...
            if (!TextUtils.isEmpty(normalizedNumber) && !TextUtils.isEmpty(minMatch)) {
                ...
                String numberE164 = PhoneNumberUtils.formatNumberToE164(
                        number, SimpleSMSApp.getApplication().getCurrentCountryIso());
                String selection;
                String[] args;
