...
public class DeleteOldMessagesService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        last.setTimeInMillis(SimpleSMSPreferences.getLong(SimpleSMSPreference.LAST_AUTO_DELETE_CHECK));

        Calendar current = Calendar.getInstance();

        // Continue if the auto delete setting is enabled, and we haven't done a purge today
        if (SimpleSMSPreferences.getBoolean(SimpleSMSPreference.AUTO_DELETE) && (last.getTimeInMillis() == 0 ||
            ...
            SimpleSMSPreferences.setLong(SimpleSMSPreference.LAST_AUTO_DELETE_CHECK, System.currentTimeMillis());
    ...
    private void deleteOldUnreadMessages(Context context) {
        ...
        calendar.add(Calendar.DAY_OF_YEAR, -Integer.parseInt(SimpleSMSPreferences.getString(SimpleSMSPreference.AUTO_DELETE_UNREAD)));
    ...
    private void deleteOldReadMessages(Context context) {
        Calendar calendar = Calendar.getInstance();
        calendar.add(Calendar.DAY_OF_YEAR, -Integer.parseInt(SimpleSMSPreferences.getString(SimpleSMSPreference.AUTO_DELETE_READ)));
        calendar.set(Calendar.HOUR_OF_DAY, calendar.getActualMaximum(Calendar.HOUR_OF_DAY));
        calendar.set(Calendar.MINUTE, calendar.getActualMaximum(Calendar.MINUTE));
        calendar.set(Calendar.SECOND, calendar.getActualMaximum(Calendar.SECOND));
